...
from cascade.randomize.pickfpuop import gen_fpufsm_instrs, inject_fpu_edge_case_vals, PROBA_INJECT_FPU_EDGE_CASE
...
def gen_basicblock(fuzzerstate):
    ...
    # Enhancement: inject IEEE 754 special values at the start of each basic block
    # when FPU is active. This fires independently of FPUFSM scheduling, guaranteeing
    # NaN/Inf/subnormal operands appear even in short programs.
    import random
    # Enhancement: inject IEEE 754 special values at start of each basic block
    if fuzzerstate.design_has_fpu and fuzzerstate.is_fpu_activated and random.random() < PROBA_INJECT_FPU_EDGE_CASE:
        try:
            new_instrobjs = inject_fpu_edge_case_vals(fuzzerstate)
            needed = 4 * len(new_instrobjs)
            if new_instrobjs and fuzzerstate.memview.get_available_contig_space(curr_alloc_cursor) > needed + BASIC_BLOCK_MIN_SPACE:
                fuzzerstate.instr_objs_seq[-1] += new_instrobjs
                fuzzerstate.memview.alloc_mem_range(curr_alloc_cursor, curr_alloc_cursor + needed)
                curr_alloc_cursor += needed
        except Exception:
            pass  # silently skip injection if memory is tight
