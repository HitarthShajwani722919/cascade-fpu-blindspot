...
def get_instr_counts():
    return dict(_instr_counts)

def reset_instr_counts():
    _instr_counts.clear()

def dump_instr_counts(filepath: str):
    os.makedirs(os.path.dirname(filepath) or '.', exist_ok=True)
    with open(filepath, 'w') as f:
        json.dump(dict(_instr_counts), f, indent=2, sort_keys=True)
    print(f"[telemetry] Saved {filepath} ({sum(_instr_counts.values())} total instructions)")
...
for _instr in ("fmadd.s", "fmsub.s", "fnmadd.s", "fnmsub.s",
               "fmin.s", "fmax.s"):
    if _instr in INSTRTYPE_INITIAL_RELATIVE_WEIGHTS[ISAInstrClass.FPU]:
        INSTRTYPE_INITIAL_RELATIVE_WEIGHTS[ISAInstrClass.FPU][_instr] = _FPU_EDGE_WEIGHT

for _instr in ("fmadd.d", "fmsub.d", "fnmadd.d", "fnmsub.d",
               "fmin.d", "fmax.d"):
    if _instr in INSTRTYPE_INITIAL_RELATIVE_WEIGHTS[ISAInstrClass.FPUD]:
        INSTRTYPE_INITIAL_RELATIVE_WEIGHTS[ISAInstrClass.FPUD][_instr] = _FPUD_EDGE_WEIGHT
