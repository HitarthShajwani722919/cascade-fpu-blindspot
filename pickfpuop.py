...
from cascade.cfinstructionclasses import CSRRegInstruction, RegImmInstruction, FloatIntRs1Instruction, ImmRdInstruction
from rv.asmutil import li_into_reg

IEEE754_SPECIAL_VALUES_F32 = [
    0x7F800000,  # +Inf
    0xFF800000,  # -Inf
    ...
    0x7F7FFFFF,  # largest finite (triggers OF)
    0x3F800000,  # 1.0
]

PROBA_INJECT_FPU_EDGE_CASE = 0.3

def inject_fpu_edge_case_vals(fuzzerstate):
    ret = []
    scratch = fuzzerstate.intregpickstate.pick_int_inputreg_nonzero()
    for frd in random.sample(range(fuzzerstate.num_pickable_floating_regs), min(1, fuzzerstate.num_pickable_floating_regs)):
        val32 = random.choice(IEEE754_SPECIAL_VALUES_F32)
        lui_imm, addi_imm = li_into_reg(val32, do_check_bounds=False)
        ret.append(ImmRdInstruction("lui",  scratch, lui_imm,  fuzzerstate.is_design_64bit))
        ret.append(RegImmInstruction("addi", scratch, scratch, addi_imm, fuzzerstate.is_design_64bit))
        ret.append(FloatIntRs1Instruction("fmv.w.x", frd, scratch, fuzzerstate.is_design_64bit))
    return ret
...
def gen_fpufsm_instrs(fuzzerstate):
    ...
    if fuzzerstate.is_fpu_activated:
        ...
        if random.random() < PROBA_INJECT_FPU_EDGE_CASE:
            ret += inject_fpu_edge_case_vals(fuzzerstate)
        return ret
